...
if test -s documents/parcellaire-"$PRODOUANE_DEBUG_PARCELLAIRE"-parcellaire.pdf && file -i documents/parcellaire-"$PRODOUANE_DEBUG_PARCELLAIRE"-parcellaire.pdf | grep -i application/pdf > /dev/null ; then echo " PDF Parcellaire Présent :)"; else echo "Erreur pas de PDF Parcellaire"; fi
