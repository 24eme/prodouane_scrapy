...
#                       }
    ...
    def login(self, response):
        ...
        formdata['url'] = 'aHR0cHM6Ly93d3cuZG91YW5lLmdvdXYuZnIvcHJvdGVjdC8='
        formdata['skin'] = 'douane'
    ...
    def multiservice(self, response):
        ...
        if os.environ.get('PRODOUANE_DEBUG'):
            with open("debug/dr_03_multiservice.html", 'wb') as f:
                f.write(response.body)
        ...
        yield scrapy.Request(url='https://www.douane.gouv.fr/ncvi-web-recolte-prodouane/connexionProdouane?code_teleservice=PORTAIL_VITI&sid=%s&app=118' % sid, callback=self.dr_accueil, meta={'departement': 0, 'commune': 0, 'annee': os.environ['PRODOUANE_ANNEE'], "cvi": cvi, "sid": sid})
    ...
    def dr_accueil(self, response):
        self.log('dr_accueil')
        if os.environ.get('PRODOUANE_DEBUG'):
            with open("debug/dr_04_dr-connexion.html", 'wb') as f:
                f.write(response.body)
    ...
    def dr_communes(self, response):
        ...
        if os.environ.get('PRODOUANE_DEBUG'):
            with open("debug/dr_05_dr-commune.html", 'wb') as f:
                f.write(response.body)
    ...
    def dr_page_1(self, response):
        self.log('dr_page_1')
        if os.environ.get('PRODOUANE_DEBUG'):
            with open("debug/dr_06_dr-page1.html", 'wb') as f:
                f.write(response.body)
    ...
    def dr_tableau_cvi(self, response):
        ...
        if os.environ.get('PRODOUANE_DEBUG'):
            with open("debug/dr_07_dr-tableau.html", 'wb') as f:
                f.write(response.body)
