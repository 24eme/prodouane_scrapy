...
#                       }
    ...
    def login(self, response):
        yield scrapy.FormRequest(url='https://pro.douane.gouv.fr/WDsession.asp', formdata={"login":os.environ['PRODOUANE_USER'],"pass":os.environ['PRODOUANE_PASS']}, callback=self.sv11_postlogin)

    def sv11_postlogin(self, response):
        yield scrapy.Request(url='https://pro.douane.gouv.fr/wdactuapplif.asp?wdAppli=118',  callback=self.sv11_postmenu)

    def sv11_postmenu(self, response):
        cvi = ''
        if 'CVI' in os.environ:
            cvi = os.environ['CVI']
        yield scrapy.Request(url='https://pro.douane.gouv.fr/wdroute.asp?btn=118&rap=3&cat=3',  callback=self.sv11_login, meta={'departement': 0, 'commune': 0, 'annee': os.environ['PRODOUANE_ANNEE'], "cvi": cvi})
    ...
    def sv11_login(self, response):
        ...
        yield scrapy.Request(url='https://pro.douane.gouv.fr/ncvi_sv11/prodouane/connexionProdouane?sid=%s&app=%s&code_teleservice=%s' % (args['sessionidT'], args['idappliT'], args['codeT']),  callback=self.sv11_connexion, meta=response.meta)
    ...
    def sv11_connexion(self, response):
...
#            f.write(response.body)
        ...
        yield scrapy.FormRequest(url='https://pro.douane.gouv.fr/ncvi_sv11/prodouane/jsp/accueilOrganisme.jsf?javax.portlet.faces.DirectLink=true', formdata=args, callback=self.sv11_communes, meta=response.meta)
...
#            f.write(response.body)
        ...
        yield scrapy.FormRequest(url='https://pro.douane.gouv.fr/ncvi_sv11/prodouane/jsp/accueilOrganisme.jsf', formdata=args, callback=self.sv11_tableau_cvi, meta=response.meta)
...
#            f.write(response.body)
        ...
        if (len(info) > id):
            ...
            if (len(info) == 30) and (nb_docs > (30 * (response.meta['page'] + 1))) :
                ...
                response.meta['id'] = 0
                yield scrapy.FormRequest(url='https://pro.douane.gouv.fr/ncvi_sv11/prodouane/jsp/accueilOrganisme.jsf', formdata=myargs, callback=self.sv11_tableau_cvi, meta=response.meta)
            else:
    ...
    def sv11_tableur_sv11(self, response):
        ...
        if (not len(response.meta['cvi'])):
            yield scrapy.FormRequest(url='https://pro.douane.gouv.fr/ncvi_sv11/prodouane/jsp/accueilOrganisme.jsf?%d,%d,%d,%d' % (response.meta['departement'], response.meta['commune'], response.meta['page'], response.meta['id']), callback=self.sv11_page_1, meta=response.meta)
