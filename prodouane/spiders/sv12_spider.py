        ...
        if (not len(response.meta['cvi'])) :
            ...
            if (len(info) == 30) and (nb_docs > (30 * (response.meta['page'] + 1))) :
                ...
                if (response.meta['nb_departements'] > response.meta['departement']):
                    yield scrapy.FormRequest(url='https://www.douane.gouv.fr/ncvi-web-sv12-prodouane/jsp/accueilOrganisme.jsf?commune=%d&dep=%d' % (response.meta['commune'], response.meta['departement']), callback=self.sv12_connexion, meta=response.meta)
        elif (len(info) > id):
